...
export async function maybeAutoGenerateDraft(input: {
  ...
  // (6) no existing draft — generate a fresh one, subject to the open-draft-review cap.
  const [openReviews] = await db
    .select({ count: sql<number>`count(*)::int` })
    .from(workItems)
    .where(sql`${workItems.campaignId} = ${campaign.id}
      and ${workItems.type} = 'draft_review_pending'
      and ${workItems.status} = 'open'`);
  if (openReviews && openReviews.count >= campaign.maxOpenDraftReviews) {
    return;
  }
