...
export async function maybeAutoGenerateDraft(input: {
  ...
  // Also stop if a generate_cold_draft job is already in flight for this org —
  // the draft row does not exist until that job runs, so the existing-draft
  // check alone would let two near-simultaneous triggers (contact + snapshot)
  // both enqueue. generateDraftCommand's idempotencyKey is time-bucketed, so
  // it does not dedup two calls a moment apart.
  ...
  // (4) active source campaign + (6) open-draft-review cap. The campaign link
  // lives on discovery_candidates (orgs carry no campaign_id).
  ...
  // All conditions hold — generate. operatorBrief is required by the schema;
  // synthesize a default the drafting agent can work from (the operator can
  // regenerate with a custom brief from the org page if they want).
  const operatorBrief =
  ...
  // No actorId — this is a system action, not an operator command, and
  // commands.actor_id is a uuid column (a "system" string is invalid).
  // source="system" marks it as auto-generated in the command audit.
