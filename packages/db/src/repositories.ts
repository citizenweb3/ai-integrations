...
export type WorkItemDetail = {
  ...
  draftId: string | null;
  // T-026BM: the thread's current warm draft (one live draft per thread, linked
  // via drafts.thread_id). Powers the work-item warm-reply panel — preview +
  // open/send/discard/regenerate. Null when none exists yet (auto-gen hasn't
  // fired, the reply isn't threaded, or the only draft was discarded). Mirrors
  // OrganizationDetail.latestDraft on the cold side.
  latestDraft: {
    id: string;
    version: number;
    status: string;
...
export async function generateDraftCommand(input: {
  ...
  actorId?: string;
  // T-026BI: auto-generation passes "system" so the command audit reflects
  // who initiated it (decision #94: commands distinguished by source). Manual
  // operator clicks keep the default "operator".
...
// T-026BI: auto-generate one cold draft for an org once it is draft-ready.
// "Draft-ready" = a published (quality-gate-passed) snapshot, at least one
// addressable contact, active facts on that snapshot, an active campaign, no
// existing draft for the org, and the campaign's open-draft-review cap not
// exhausted. Generation produces a draft only — approval/send stay manual
// (zero-autosend #58/#59). Reuses generateDraftCommand verbatim (it resolves
// the primary/fallback contact and opens its own tx), so this helper runs
// standalone after a job's tx commits, not inside a router tx. Idempotent:
// the no-existing-draft check plus generate_draft:<orgId> concurrencyKey stop
// duplicates when both the contact and the snapshot events fire.
export async function maybeAutoGenerateDraft(input: {
  organizationId: string;
}): Promise<void> {
  const db = getDb();

  // (1) published snapshot with (3) active facts.
  const [snapshot] = await db
    .select({ id: researchSnapshots.id })
    .from(researchSnapshots)
    .where(sql`${researchSnapshots.organizationId} = ${input.organizationId}
  ...
  const [factRow] = await db
    ...
    .from(researchFacts)
    .where(sql`${researchFacts.snapshotId} = ${snapshot.id}
      and ${researchFacts.status} = 'active'`);
  if (!factRow || factRow.count === 0) return;

  // (2) at least one addressable contact (email present).
  const [contactRow] = await db
    .select({ count: sql<number>`count(*)::int` })
    .from(contacts)
    .where(sql`${contacts.organizationId} = ${input.organizationId}
      and ${contacts.email} is not null`);
  if (!contactRow || contactRow.count === 0) return;

  // (5) no existing draft for the org (any live status — only a discarded
  // draft frees the slot for regeneration). Drafts carry no organization_id;
  // they link to the org through their contact (draft.contact_id ->
  // contacts.organization_id).
  // (5) the org's current live draft (status + version, any non-discarded
  // status). Drafts carry no organization_id; they link to the org through
  // their contact (draft.contact_id -> contacts.organization_id). Most-recent
  // wins. Drives the refresh-vs-flag decision below.
  const [existingDraft] = await db
    .select({ id: drafts.id, status: drafts.status, version: drafts.version })
    .from(drafts)
    .innerJoin(contacts, eq(contacts.id, drafts.contactId))
    .where(sql`${contacts.organizationId} = ${input.organizationId}
      and ${drafts.status} <> 'discarded'`)
    .orderBy(desc(drafts.createdAt))
    .limit(1);

  // Stop if a generate_cold_draft job is already in flight for this org — the
  // draft row does not exist until that job runs, so the existing-draft check
  // alone would let two near-simultaneous triggers (contact + snapshot) both
  // enqueue. generateDraftCommand's idempotencyKey is time-bucketed, so it does
  // not dedup two calls a moment apart. Also prevents a refresh racing a draft
  // that is still being written.
  const [inflightJob] = await db
    .select({ id: jobs.id })
    .from(jobs)
    .where(sql`${jobs.jobType} = 'job.generate_cold_draft'
      and ${jobs.targetEntityId} = ${input.organizationId}
      and ${jobs.status} in ('queued', 'leased', 'running')`)
    .limit(1);
  if (inflightJob) return;

  // (4) active source campaign. The campaign link lives on discovery_candidates
  // (orgs carry no campaign_id). Needed by every path below.
  const [campaign] = await db
    ...
    .from(campaigns)
    .where(sql`${campaigns.id} = (
      select ${discoveryCandidates.campaignId}
      from ${discoveryCandidates}
      where ${discoveryCandidates.matchedOrganizationId} = ${input.organizationId}
      limit 1
    )`)
    .limit(1);
  if (!campaign || campaign.status !== "active") return;

  // operatorBrief is required by the schema; synthesize a default the drafting
  // agent can work from (the operator can regenerate with a custom brief from
  // the org page if they want). No actorId — system action; commands.actor_id
  // is a uuid column. source="system" marks it as auto-generated.
  const operatorBrief =
    `Auto-generated cold outreach for ${campaign.name}. Use the research ` +
    `snapshot facts to ground the email; keep it concise with one clear ask.`;

  // T-026BP: refresh-on-new-research. This helper fires whenever a fresh
  // published snapshot lands for the org. When a draft already exists:
  //   - a PRISTINE AI draft (status=draft, version=1, never edited / revised) is
  //     regenerated from the newer snapshot — same single-live-draft philosophy
  //     as the initial auto-gen;
  //   - an operator-INVESTED draft (manually edited / AI-revised → version>1, or
  //     approved / sending / sent / failed) is NOT clobbered. Instead a
  //     `draft_stale_research_updated` work item flags it so the operator can
  //     review and regenerate on their own terms.
  if (existingDraft) {
    const pristine = existingDraft.status === "draft" && existingDraft.version === 1;
    if (pristine) {
      await generateDraftCommand({
        source: "system",
        payload: {
      ...
      await db.transaction(async (tx) => {
        await createWorkItem(tx, {
          type: "draft_stale_research_updated",
          priority: 50,
          ...
          title: "Outreach draft may be stale — research updated",
          summary:
            "Newer research landed for this organization, but the current draft " +
            "was already edited or approved, so it was not auto-updated. Review " +
            "and regenerate if the new facts matter.",
          reasonCode: "research_updated_draft_stale",
          actionLabel: "Review draft",
          dedupeKey: `draft_stale:${existingDraft.id}`
        });
      });
    }
    return;
  ...
  // (6) no existing draft — generate a fresh one. There is intentionally NO
  // open-draft-review cap here: a campaign asked to find N companies should
  // produce a draft for every addressable, researched org. Backpressure by
  // pausing draft generation at 25 was surprising ("found 100, got 25 drafts")
  // and silently stalled the funnel. maxOpenDraftReviews is kept on the schema
  // for compatibility but no longer gates auto-draft.
  await generateDraftCommand({
    source: "system",
    payload: {
      organizationId: input.organizationId,
...
export async function generateWarmDraftCommand(input: {
  ...
  actorId?: string;
  // T-026BM: "system" marks an auto-generated warm draft (no operator actor),
  // mirroring generateDraftCommand. Defaults to "operator".
  source?: CommandSource;
  ...
  return db.transaction(async (tx) => {
    ...
    // T-026BM regenerate: discard the thread's current editable draft first so
    // the thread keeps a single live warm draft instead of stacking versions.
    // Only 'draft'-status rows are discarded — approved/sent drafts are
    // immutable history. Mirrors the cold-side replaceExisting.
    if (payload.replaceExisting) {
      await tx
        .update(drafts)
        .set({ status: "discarded", updatedAt: new Date() })
        .where(sql`${drafts.status} = 'draft' and ${drafts.threadId} = ${thread.id}`);
    }
    ...
    // The warm idempotency key is thread+inbound+intentHash (not time-bucketed
    // like the cold side). On a regenerate (replaceExisting) the operator wants
    // a brand-new draft, so seed the hash with a timestamp — otherwise a second
    // regenerate with the same default intent would dedup against the prior
    // command and silently produce nothing.
    const intentSeed = payload.replaceExisting
      ? `${payload.replyIntent}:${new Date().toISOString()}`
      : payload.replyIntent;
...
// T-026BM: default reply intent for an auto-generated / one-click warm draft.
// The operator does not type an intent on the auto path; the drafting agent
// receives the full transcript + latest inbound + research and infers the
// specifics. Exported so the UI's Generate/Regenerate buttons reuse the exact
// same text the auto path uses.
export const DEFAULT_WARM_REPLY_INTENT =
  "Reply to the contact's latest message: keep it warm and concise, answer " +
  "anything they asked, and move the conversation toward a concrete next step " +
  "(a call or a clear answer). Ground it in the thread and the org's research.";

// T-026BM: auto-generate one warm draft when a reply lands in a warm class,
// mirroring maybeAutoGenerateDraft on the cold side. Runs standalone after the
// classify tx commits (generateWarmDraftCommand opens its own tx). Idempotent:
// the no-live-draft check + the inflight-job check stop duplicates, and the warm
// command's per-thread concurrency lock serializes any race.
export async function maybeAutoGenerateWarmDraft(input: {
  inboundMessageId: string;
}): Promise<void> {
  const db = getDb();

  // Resolve the thread from the inbound. No thread → nothing to reply into (the
  // operator must attach it first), so skip auto-gen — the work item still
  // exists for manual handling.
  const [inbound] = await db
    .select({ threadId: inboundMessages.threadId })
    .from(inboundMessages)
    .where(eq(inboundMessages.id, input.inboundMessageId))
    .limit(1);
  if (!inbound?.threadId) return;
  const threadId = inbound.threadId;

  // Thread must still be actionable (the command re-checks, but skip early to
  // avoid a needless command row).
  const [thread] = await db
    .select({ status: threads.status, organizationId: threads.organizationId })
    .from(threads)
    .where(eq(threads.id, threadId))
    .limit(1);
  if (!thread || (thread.status !== "open" && thread.status !== "active")) return;
  if (!thread.organizationId) return;

  // No existing live draft for this thread (only a discarded draft frees the
  // slot for regeneration).
  const [existingDraft] = await db
    .select({ id: drafts.id })
    .from(drafts)
    .where(sql`${drafts.threadId} = ${threadId} and ${drafts.status} <> 'discarded'`)
    .limit(1);
  if (existingDraft) return;

  // No warm-draft job already in flight for this thread — the draft row does
  // not exist until that job runs, so the existing-draft check alone would let
  // two near-simultaneous triggers both enqueue.
  const [inflightJob] = await db
    .select({ id: jobs.id })
    .from(jobs)
    .where(sql`${jobs.jobType} = 'job.generate_warm_draft'
      and ${jobs.targetEntityId} = ${threadId}
      and ${jobs.status} in ('queued', 'leased', 'running')`)
    .limit(1);
  if (inflightJob) return;

  // No actorId — system action; commands.actor_id is a uuid column.
  await generateWarmDraftCommand({
    source: "system",
    payload: { threadId, replyIntent: DEFAULT_WARM_REPLY_INTENT }
  });
}
...
export async function completeRefreshResearchSnapshotJob(input: {
  ...
  if (routerResult) {
    ...
    // T-026BI: the snapshot may have just become published — try auto-drafting
    // (no-op unless an addressable contact also exists, etc.).
    await maybeAutoGenerateDraft({ organizationId: input.organizationId });
  }
...
export async function completeDiscoverContactsJob(input: {
  ...
  // T-026BI: a contact may have just become addressable — try auto-drafting
  // (no-op unless the snapshot is also published, facts active, etc.).
  await maybeAutoGenerateDraft({ organizationId: input.organizationId });
}
...
export async function routeClassifyReplyOutcome(input: {
  ...
  // T-026BM: warm classes get a draft auto-generated immediately, mirroring the
  // cold-draft flow — the operator opens the reply and finds a ready warm draft
  // to review / regenerate / discard rather than a blank intent form. Runs
  // after the classify tx commits (the warm command opens its own tx); the
  // helper is self-gating + idempotent, so a non-warm class or a missing thread
  // is a no-op.
  if (
    winner
    ...
    await maybeAutoGenerateWarmDraft({ inboundMessageId: input.inboundMessageId });
  }
  ...
  switch (input.replyClass) {
    ...
    case "neutral": {
      const created = await createWorkItem(tx, {
        ...
        summary: "Inbound reply classified as a safe class. A warm draft is generated automatically; the operator reviews, regenerates, or discards it.",
...
export async function completeResearchMoreJob(input: {
  ...
  if (routerResult) {
    ...
    // T-026BI: snapshot may have just become published — try auto-drafting.
    await maybeAutoGenerateDraft({ organizationId: input.organizationId });
  }
...
async function createWorkItem(
  ...
  input: {
    ...
    dedupeKey: string;
    inboundMessageId?: string;
    // T-026BP: optional entity links so a work item can point at the draft /
    // org / campaign it concerns (powers the detail page's draft link + saved
    // view campaign filter). Omitted callers behave exactly as before.
    draftId?: string;
    organizationId?: string;
    campaignId?: string;
  }
...
export async function getWorkItemDetail(id: string): Promise<WorkItemDetail | null> {
  ...
  // T-026BM: the thread's current warm draft (if any), for the warm-reply panel.
  // leftJoin contacts — a warm draft may have an unresolved contact (null
  // contactId), and an innerJoin would drop it. Latest non-discarded wins.
  const latestDraftRows = inboundMessage?.threadId
    ? await db
        .select({
          id: drafts.id,
          version: drafts.version,
        ...
        .from(drafts)
        .leftJoin(contacts, eq(contacts.id, drafts.contactId))
        .where(sql`${drafts.threadId} = ${inboundMessage.threadId} and ${drafts.status} <> 'discarded'`)
        .orderBy(desc(drafts.createdAt))
        .limit(1)
    : [];
  const latestDraft: WorkItemDetail["latestDraft"] = latestDraftRows[0]
    ? {
        ...
        autosendReadiness: latestDraftRows[0].autosendReadiness
      }
    : null;
...
export async function getOrganizationDetail(id: string): Promise<OrganizationDetail | null> {
  ...
  // Most-recent non-discarded draft drives the org-page draft panel. Built
  // with the query builder (not raw execute) so it returns rows uniformly.
  const latestDraftRows = await db
    ...
    .from(drafts)
    .innerJoin(contacts, eq(contacts.id, drafts.contactId))
    .where(and(eq(contacts.organizationId, id), ne(drafts.status, "discarded")))
    .orderBy(desc(drafts.createdAt))
    .limit(1);
