            ...
            {steps.map((step, i) => (
              <div key={step.title} className="flex items-start">
                <div className="flex-1">
                  ...
                  {/* Content */}
                  <div className="pr-6">
                    <p className={`text-base font-bold mb-2 ${step.highlight ? 'text-[#2FFBF7]' : 'text-[#E6E6E6]'}`}>
                      {step.title}
                    </p>
                    <p className="text-sm font-light text-[hsl(220,10%,64%)] leading-relaxed">{step.detail}</p>
                    {step.metrics && (
                      <ul className="mt-3 flex flex-col gap-1.5">
                        {step.metrics.map((m) => (
                          <li key={m} className="text-sm font-light text-[#E6E6E6]">
                            <span className="text-[#2FFBF7] mr-2">—</span>
                            {m}
                          </li>
                        ))}
                      </ul>
          ...
          {steps.map((step, i) => (
            <div key={step.title} className="flex">
              ...
              <div className="flex flex-col items-center mr-5">
                ...
                {i < steps.length - 1 && <div className="w-px flex-1 bg-white/10" />}
              </div>

              {/* Content */}
              <div className="pb-8 pt-0.5">
                <p className={`text-base font-bold mb-1 ${step.highlight ? 'text-[#2FFBF7]' : 'text-[#E6E6E6]'}`}>
                  {step.title}
                </p>
                <p className="text-sm font-light text-[hsl(220,10%,64%)] leading-relaxed">{step.detail}</p>
                {step.metrics && (
