...
- [ ] `getCampaignProgress(campaignId)` returns `{contactsAccepted, draftsGenerated, draftsApproved, sent, replied, replyClassCounts, lastActivityAt}`
- [ ] `/campaigns` lists campaigns with progress numbers
- [ ] `/campaigns/[id]` shows the same breakdown

## T-023 — `getOperationsCounters` 1-second cache

- [ ] In-process cache with 1s TTL
- [ ] Concurrent requests within TTL share one DB hit
- [ ] Cache key invalidates on `system_state` change (optional bonus)
...
- [ ] Counters listed in TICKETS.md T-025 all present
- [ ] OTLP exporter enabled when `OTEL_EXPORTER_OTLP_ENDPOINT` is set; silent when unset
- [ ] `correlationId` propagated as trace baggage
